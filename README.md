...
d. Our human-centered designed techniques and tools included:		 - Using JIRA as a collaboration tool for the team to communicate and understand needs, and to log Product Backlog, work performed, and issues found;	 - Gathering requirements for user stories; 
	 ...
	 - m, n. We used GitHub for configuration management. Continuous monitoring and alerts were enabled by AWS tools for CPU utilization and failure of any status check .	 - p. Our Developer created a document describing the steps to deploy the prototype on another machine.
